...
/**
 * Component ForgotPassword - Trang yêu cầu khôi phục mật khẩu.
 * Nhập Email hoặc Username, gửi yêu cầu đến server để nhận mã OTP khôi phục mật khẩu
 * về email đăng ký của tài khoản, sau đó tự động chuyển người dùng sang trang xác thực OTP.
 */
export default function ForgotPassword() {
  // State lưu trữ Email hoặc Username do người dùng nhập vào
  const [email, setEmail] = useState("");
  // State hiển thị thông báo thành công sau khi gửi mã OTP thành công
  const [message, setMessage] = useState("");
  // State hiển thị thông điệp lỗi
  const [error, setError] = useState("");
  // Trạng thái chờ gọi API
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();

  // Hàm xử lý gửi yêu cầu quên mật khẩu khi submit form
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); // Ngăn hành vi reset trang mặc định
    setMessage(""); 
    setError(""); 
    setLoading(true); // Bật hiệu ứng chờ
    
    try {
      // Gọi API yêu cầu khôi phục mật khẩu thông qua email/username
      const res = await authService.forgotPassword({ email: email.trim() });
      
      if (res.success && res.otpTxId) {
        // Nếu thành công và nhận được mã giao dịch khôi phục mật khẩu:
        // 1. Hiển thị thông báo thành công cho người dùng
        setMessage("Verification code sent! Redirecting...");
        // 2. Chuyển hướng người dùng sang trang verify-forgot sau 800ms để nhập mã OTP khôi phục
        setTimeout(() => {
          navigate(`/verify-forgot?otpTxId=${res.otpTxId}`);
        }, 800);
      } else {
        // Nếu backend báo lỗi nghiệp vụ
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100vh", overflow: "hidden" }}>
      {/* Phông nền ảnh chiến mã tương tự */}
      ...
      <div style={{ position: "relative", zIndex: 10, display: "flex", alignItems: "center", justifyContent: "center", width: "100%", height: "100%", padding: "0 1rem" }}>
        <div style={{ width: "100%", maxWidth: "28rem" }}>
          ...
          {/* Khung Card chứa form Reset Password */}
          <div style={{ background: "rgba(21,19,16,0.95)", backdropFilter: "blur(8px)", border: "1px solid #2a2825", borderRadius: "0.5rem", padding: "2rem", boxShadow: "0 25px 50px rgba(0,0,0,0.5)" }}>
            ...
            {/* Banner hiển thị thông báo thành công */}
            {message && (
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {/* Trường nhập Email hoặc Username để tìm kiếm tài khoản */}
              <div>
